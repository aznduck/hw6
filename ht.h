...
{
    if(((double)deletedItems) / (table_.size()) >= alpha)
        resize();
...
{
    mIndex_++;
    if(mIndex_ >= sizeof(CAPACITIES)) throw std::logic_error("No More Capacities");
    

    int index = 0;
    std::vector<HashTable<K, V, Prober, Hash, KEqual>::HashItem*> itemsToAdd(numItems);
    for(size_t i = 0; i < table_.size(); i++)
    {
        if(!table_[i])
        {
    ...
    for (size_t i = 0; i < itemsToAdd.size(); i++) {
        HASH_INDEX_T h = this->probe(itemsToAdd[i] -> item.first);
        table_[h] = itemsToAdd[i];
    }
